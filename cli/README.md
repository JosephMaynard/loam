...
loam [options]
  ...
  --encrypt         Encrypt the database at rest with SQLCipher. The passphrase
                    ...
                    for it (not echoed). An empty answer, or no terminal to
                    prompt on, uses an ephemeral RAM-only key that is discarded
                    on exit.
  --encrypt ephemeral

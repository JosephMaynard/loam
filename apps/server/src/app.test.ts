...
describe("node-to-node sync", () => {
  ...
  it("re-syncs channel metadata (rename + archive) to a peer that already imported it (C1)", async () => {
    const source = await makeApp({ sync: { enabled: true, peers: [], intervalMs: 3_600_000 } });
    const sourceAdmin = await newSession(source);
    await source.server.inject({
      method: "POST",
    ...
    // First sync imports the channel as-is.
    await runSync(puller, pullerAdmin.cookie);
    const beforeChannel = puller.store.loadChannels().find((channel) => channel.id === "relief-ops");
    expect(beforeChannel?.name).toBe("Relief Ops");
    expect(beforeChannel?.archived ?? false).toBe(false);

    // Rename + archive on the SOURCE.
    const patched = await source.server.inject({
      method: "PATCH",
    ...
    // A second sync propagates the metadata change — the pre-fix behaviour skipped any channel already held.
    await runSync(puller, pullerAdmin.cookie);
    const afterChannel = puller.store.loadChannels().find((channel) => channel.id === "relief-ops");
    expect(afterChannel?.name).toBe("Relief Ops (closed)");
    expect(afterChannel?.archived).toBe(true);
    expect(afterChannel?.updatedAt).toBeDefined();

    // Newer-wins is one-directional: a stale peer copy never clobbers a fresher local edit. Re-running with
    // no further source change is a no-op (the name stays the edited one).
    await runSync(puller, pullerAdmin.cookie);
    expect(puller.store.loadChannels().find((channel) => channel.id === "relief-ops")?.name).toBe(
      "Relief Ops (closed)",
    );
  });

...
import { createStoreLifecycle, type WipePhase } from "./store-lifecycle.js";
import { createSyncEngine } from "./sync.js";
import { resolveLanIPv4 } from "./net.js";

import type { SocketClient, SocketSession, AppData, ClientEvent, AppOptions, LoamApp } from "./types.js";
...
import { makeUser, makeSessionUserId, makeSessionToken, makeIdentityToken, hashIdentityToken, makeAdminSetupCode, encodeCookieValue, readCookie } from "./identity.js";
import { newAvatarImageId, newAttachmentId, isImageAttachmentMime, sanitizeAttachmentName, attachmentFileName, parseAttachmentFileName, attachmentMaxBytes, attachmentFileMaxBytes, avatarImageExtension, parseAvatarImageId, avatarImageHasExpectedSignature } from "./media.js";
...
export async function buildApp(options: AppOptions): Promise<LoamApp> {
  ...
  const lifecycle = createStoreLifecycle({
    ...
    options,
    log: server.log,
    persistConfigForRestart: (config) => persistConfigForRestart(config),
    wipeRestartHook: () => wipeRestartHook(),
  ...
  const { llmEnabled, ensureBotUser, createAssistantResponse } = llm;
  const mesh = createMeshLayer(rt);
  const { meshIdentities, loadMeshIdentities, meshContacts, loadMeshContacts, ensureMeshIdentity, ensureAllMeshIdentities, acceptSealedFromPeer, addMeshContact, meshIdentityCard, sendSealed, reapExpiredSealed } = mesh;
  const sync = createSyncEngine(rt, mesh);
  const { peerSyncStatus, peerTransportSessions, isSyncableMessage, buildSyncDigest, retryMissingAttachments, runSyncLoop, syncStatusReport } = sync;
  ...
  async function executeKillSwitchBody(): Promise<KillSwitchResult> {
    ...
    if (dbState.encryptionEnabled && fixedKeyMode) {
    ...
    if (dbState.encryptionEnabled && fixedKeyMode) {
    ...
    } else if (dbState.encryptionEnabled) {
      ...
      if (ephemeralDbKey) {
        ...
        dbState.dbKey = randomBytes(32).toString("hex");
      }
      store = openLoamStore();
